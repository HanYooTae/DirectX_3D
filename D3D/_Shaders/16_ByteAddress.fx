...
struct ComputeInput
{
    uint3 GroupID : SV_GroupID;     // Dispatch에서 현재 실행 중인 나의 스레드ID(Vector3)
    uint3 GroupThreadID : SV_GroupThreadID;     // 그룹 내 로컬ID(Vector3)
    uint3 DispatchThreadID : SV_DispatchThreadID;       // 전체 내에서 글로벌 스레드ID(Vector3)
